  ...
  return (
    <div className="flex min-h-screen bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      <div className="relative z-10 flex-1 w-full lg:w-auto">
        ...
        {/* Page Header - Hidden for Dashboard page */}
        {!isDashboardPage && (
          <div className="bg-white px-4 py-3 dark:bg-slate-950 sm:px-6 md:px-10 sm:py-4">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white truncate">{pageName}</h1>
            </div>
          </div>
        )}
        <main className={`bg-white dark:bg-slate-950 ${isDashboardPage ? '' : 'space-y-10 px-6 pb-16 pt-6 sm:px-10'}`}>
          <Outlet />
        </main>
      </div>
    </div>
