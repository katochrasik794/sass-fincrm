...
export default function PaymentMethods() {
  const methods = [
    { id: 1, name: 'Credit Card', type: 'Card', status: 'active', icon: CreditCard },
    { id: 2, name: 'Bank Transfer', type: 'Bank', status: 'active', icon: Building },
    { id: 3, name: 'Cryptocurrency', type: 'Crypto', status: 'active', icon: CreditCard },
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Payment Methods</h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">Manage payment methods for deposits</p>
        </div>
          ...
          return (
            <div key={method.id} className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300">{method.status}</span>
              </div>
