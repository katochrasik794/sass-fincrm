...
export default function InvestCopiers() {
  const [searchQuery, setSearchQuery] = useState('')

  const copiers = [
    { id: 1, name: 'John Doe', master: 'Growth Fund Master', invested: '$5,000', returns: '+$1,225', status: 'active' },
    { id: 2, name: 'Jane Smith', master: 'Tech Index Master', invested: '$3,200', returns: '+$582', status: 'active' },
    { id: 3, name: 'Mike Johnson', master: 'Forex Pro Master', invested: '$2,500', returns: '+$395', status: 'active' },
  ]

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Investment Copiers</h1>
        <p className="text-slate-600 dark:text-slate-400 mt-1">Manage investment copiers</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-800/50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Invested</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {copiers.map((copier) => (
                <tr key={copier.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{copier.invested}</td>
                  <td className="px-6 py-4 font-semibold text-emerald-600 dark:text-emerald-400">{copier.returns}</td>
