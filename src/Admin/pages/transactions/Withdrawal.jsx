...
import { Search, Filter, Download, CheckCircle, XCircle, Clock, DollarSign, AlertCircle } from 'lucide-react'
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-1">Manage all withdrawal requests</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
          <p className="text-sm text-slate-600 dark:text-slate-400">Total Withdrawn</p>
          <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">$85K</p>
          ...
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-800/50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">Requested</th>
              ...
              {withdrawals.map((withdrawal) => (
                <tr key={withdrawal.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4">
                    <p className="font-semibold text-slate-900 dark:text-white">{withdrawal.transactionId}</p>
                  </td>
                  <td className="px-6 py-4 text-slate-900 dark:text-white">{withdrawal.clientName}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1">
                      <DollarSign className="h-4 w-4 text-red-600 dark:text-red-400" />
                      <span className="font-bold text-red-600 dark:text-red-400">{withdrawal.amount}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{withdrawal.method}</td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{withdrawal.fee}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${
                        withdrawal.status === 'completed' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300' :
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {withdrawal.rejectionReason && (
                        <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title={withdrawal.rejectionReason}>
